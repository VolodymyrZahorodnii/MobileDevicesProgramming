...
fun SalaryDisplay(salary: Salary, modifier: Modifier = Modifier) {
    ...
    Column(
        ...
        AnimatedContent(
            ...
            transitionSpec = {
                val enter = expandHorizontally(
                    animationSpec = tween(delayMillis = 700)
                )
                val exit = shrinkHorizontally()
...
fun ReturnButton(
    ...
    Column(
        ...
        Button(onClick = onClick) {
            Text(stringResource(R.string.salary_screen_return_button_label), style = MaterialTheme.typography.titleLarge)
        }
    }
}
